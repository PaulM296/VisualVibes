...
const getCommentsCountByPostId = async (postId: string) => {
  try {
    const response = await apiClient.get(
      `/comments/post/${postId}/comments/total`
    );
...
const getPostComments = async (
  ...
  pageIndex: number = 1,
  pageSize: number = 10
): Promise<PaginationResponse<ResponseComment>> => {
  try {
...
const moderateComment = async (commentId: string) => {
  await apiClient.put(`/comments/${commentId}/moderate`);
};

const unmoderateComment = async (commentId: string) => {
  await apiClient.put(`/comments/${commentId}/unmoderate`);
};
