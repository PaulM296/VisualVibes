...
const getUserFollowing = async (
  userId: string
): Promise<UserFollowerInterface[]> => {
  const response = await apiClient.get<UserFollowerInterface[]>(
...
const blockUser = async (userId: string): Promise<void> => {
  await apiClient.put(`/users/${userId}/block`);
};

const unblockUser = async (userId: string): Promise<void> => {
  await apiClient.put(`/users/${userId}/unblock`);
};
